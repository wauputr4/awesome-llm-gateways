...
AI Model Gateway is a self-hosted LLM operations gateway that combines OpenAI Chat Completions, Anthropic Messages, and OpenAI Responses-compatible entry points with provider routing, fallback, rate limiting, telemetry, benchmarking, diagnostics, config publishing, and rollback workflows. It is designed for teams that want provider keys, routing policy, telemetry, and operational controls inside their own environment rather than a hosted broker.
...
- Admin and CLI workflows cover config preview, diff, publish, audit, rollback, provider probes, diagnostics, and verified update operations.
